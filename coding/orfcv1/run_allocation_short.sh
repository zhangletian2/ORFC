...
mkdir -p "$OUT"/logs
# Independent K-means modes supply data-driven candidates for nested splitting.
CUDA_VISIBLE_DEVICES=${GPU_REPAIR:-4} "$PY" p1_fixed_rate.py prepare \
  ...
  --max-vectors 62500 --kmeans-iters 50 --seed 42 \
...
CUDA_VISIBLE_DEVICES=${GPU_REPAIR:-4} "$PY" allocation_train.py repair \
  ...
  --split-scale 0.001 --margin 0.001 \
  --monotonic-tolerance 0.001 --minimum-high-rate-gain 0.001 \
