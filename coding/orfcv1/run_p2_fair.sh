...
MEAS=${MEAS:-$P1/response/measurement_R192.npz}
CAL=${CAL:-$P1/response/calibration_R192.npz}
...
common=(
  ...
  --parameters "$PARAMETERS" --assignment "$ASSIGNMENT" --anchor-weight 1
  --images "$TRAIN_IMAGES" --allocations "$ALLOCATIONS" --allocation-chunk 4
  --hard-images "$HARD_IMAGES" --hard-batch-size 4 --hard-allocation-chunk 8
  --steps "$STEPS" --refresh-steps 0 --lr "$LR" --checkpoint-every 1
...
run_arm "${GPU_MEAN:-0}" mean 0 & p0=$!
run_arm "${GPU_R025:-1}" r025 0.25 & p1=$!
run_arm "${GPU_R050:-3}" r050 0.50 & p2=$!
run_arm "${GPU_R100:-4}" r100 1.00 & p3=$!
wait "$p0" "$p1" "$p2" "$p3"
...
eval_arm "${GPU_MEAN:-0}" mean & q0=$!
eval_arm "${GPU_R025:-1}" r025 & q1=$!
eval_arm "${GPU_R050:-3}" r050 & q2=$!
eval_arm "${GPU_R100:-4}" r100 & q3=$!
wait "$q0" "$q1" "$q2" "$q3"
